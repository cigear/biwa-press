...
<Dialog.Root bind:open>
  <Dialog.Trigger
    class="flex items-center justify-center rounded-md border border-border p-2 text-foreground transition hover:bg-secondary"
  ...
  <Dialog.Portal>
    ...
    <Dialog.Content
      class="fixed left-1/2 top-24 z-50 w-[min(640px,calc(100vw-2rem))] -translate-x-1/2 rounded-lg bg-background p-4 shadow-xl border border-border"
    >
      <Dialog.Title class="text-sm font-semibold text-foreground">{text.searchTitle}</Dialog.Title>
      <input
        bind:value={query}
        class="mt-4 w-full rounded-md border border-border bg-background text-foreground px-3 py-2 text-base sm:text-sm outline-none ring-brand/20 transition focus:border-brand focus:ring-4"
        placeholder={text.searchPlaceholder}
      />

      {#if query.trim().length === 0}
        <p class="mt-3 text-sm text-muted-foreground">{text.searchHint}</p>
      {:else if results.length === 0 && !isLoading}
        <p class="mt-3 rounded-md border border-border px-3 py-3 text-sm text-muted-foreground">
