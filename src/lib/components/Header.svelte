...
<header class="sticky top-0 z-30 border-b border-zinc-200 bg-white/90 backdrop-blur">
  <div class="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
    <div class="flex items-center gap-4">
      ...
      <a href={`/${locale}/docs/guide/getting-started`} class="text-sm font-semibold text-zinc-950">
        {site.title}
      </a>
    </div>
...
{#if open}
  <div class="fixed inset-0 z-40 lg:hidden">
    ...
    <div class="fixed left-0 top-0 h-full w-64 bg-white p-4 shadow-lg">
      <button class="mb-4" onclick={() => open = false} aria-label="Close menu">
        <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <nav class="space-y-4">
        {#each nav as item}
          <a class="block text-sm text-zinc-600 transition hover:text-zinc-950" href={item.href} onclick={() => open = false}>
            {item.title}
          </a>
        {/each}
      </nav>
      ...
      <div class="mt-8">
        {#each getSidebar(locale) as group}
          <h3 class="mb-2 text-sm font-semibold">{$t(group.title)}</h3>
          <ul class="space-y-2">
            {#each group.items as item}
              <li><a href={item.href} class="text-sm text-zinc-600" onclick={() => open = false}>{$t(item.title)}</a></li>
            {/each}
          </ul>
        {/each}
      </div>
