  ...
  renderer(this: any, token) {
    ...
    return `
      <div class="mermaid-container my-6 flex flex-col items-center" data-mermaid-src="${encodeURIComponent(code)}">
        <div class="mermaid-render-target hidden w-full justify-center bg-zinc-50 rounded-xl p-8 border border-zinc-200 shadow-inner"></div>
        <div class="mermaid-fallback hidden w-full">
          <pre class="p-4 bg-[#0d1117] rounded-xl overflow-x-auto text-sm font-mono text-[#e6edf3] border border-white/5 shadow-sm">
<code>${code}</code>
          </pre>
          <p class="text-xs text-center opacity-30 mt-2 font-sans">${hint}</p>
        </div>
      </div>
    `;
  }
